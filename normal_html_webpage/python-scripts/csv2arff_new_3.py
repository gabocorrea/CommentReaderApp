...
def getPatternCountList( text , regexList):
	...
	for regex in regexList:
		...
		if p.search(text) == None:
			...
			count+=1 ############## this does not count repeated keywords. phrase 'may must may' counts may and must giving 2.
...
with open(fileIn_name[:-4]+sparce_str_file_name+dont_use_revise_directive_type_name+"_3.arff","w") as fdout:
	fdout.write("% 1. Title: Directive Keywords present in file "+ fileIn_name +"\n\n")
	fdout.write("@RELATION directive_keywords_3\n\n")

	fdout.write("@ATTRIBUTE id NUMERIC\n")
	fdout.write("@ATTRIBUTE keyword_count NUMERIC\n")
	if dont_use_revise_directive_type:
